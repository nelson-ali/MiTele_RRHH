...

{
    ...
    private function defineViewCols()
    {
        ...
        $listaDeColumnas = array(
            "hdnNombres", "hdnCi", "hdnExpd", "hdnGenero", "hdnEdad", "hdnFechaNac", "hdnFechaCumple", "hdnEstadoDescripcion", "hdnActivo",
            "hdnUbicacion", "hdnCondicion", "hdnGerencia", "hdnDepartamento", "hdnArea", "hdnProcesoContratacion", "hdnFuente", "hdnNivelSalarial",
            "hdnCargo", "hdnHaber", "hdnFechaIng", "hdnFechaIni", "hdnFechaIncor", "hdnFechaFin", "hdnFechaBaja", "hdnMotivoBaja", "hdnAntiguedad", "hdnObservacion",
        );
    ...
    public function listpagedAction()
    {
        ...
        if ($gestion >= 0) {
            $resul = $obj->getPaged($opcion, $gestion, $where, '', $start, $pagesize);
            if ($resul->count() > 0) {
                foreach ($resul as $v) {
                    $total_rows = $v->total_rows;
                    $antiguedad = "";
                    if ($v->fecha_ing != "") {
                        $fecha1 = date("Y-m-d 00:00:00", strtotime($v->fecha_ing));
                        if ($v->fecha_baja == "") {
                            $fecha2 = date("d-m-Y");
                            $fecha2 = $this->sumarDiasFecha($fecha2, 1) . " 00:00:00";
                        } else {
                            $fecha2 = date("d-m-Y", strtotime($v->fecha_baja));
                            $fecha2 = $this->sumarDiasFecha($fecha2, 1) . " 00:00:00";
                        }
                        $antiguedad = $this->tiempoTranscurridoFechas($fecha1, $fecha2);
                    }
                    $relaborales[] = array(
                        'nro_row' => 0,
                        'id_relaboral' => $v->id_relaboral,
    ...
    public function listactivosengestionAction()
    {
        ...
        $resul = $obj->getAllInGestion($gestion, $sql);
        //comprobamos si hay filas
        if ($resul->count() > 0) {
            foreach ($resul as $v) {
                $relaboral[] = array(
    ...
    public function exportpdfAction($n_rows, $gestion_consulta, $columns, $filtros, $groups, $sorteds)
    {
        ...
        $generalConfigForAllColumns = array(
            ...
            'antiguedad' => array('title' => 'Antiguedad', 'width' => 18, 'align' => 'J', 'type' => 'varchar'),
        ...
        if (count($widthsSelecteds) > 0) {
            ...
            $resul = $obj->getPaged(1, $gestion_consulta, $where, $groups, 0, 0);
            $relaboral = array();
            foreach ($resul as $v) {
                $antiguedad = "";
                if ($v->fecha_ing != "") {
    ...
    public function exportactivospdfAction($n_rows, $gestion_consulta, $columns, $filtros, $groups, $sorteds)
    {
        ...
        if (count($widthsSelecteds) > 0) {
            ...
            $where = str_replace("'", "''", $where);
            $resul = $obj->getPaged(0, $gestion_consulta, $where, $groups, 0, 0);
            $relaboral = array();
            foreach ($resul as $v) {
                $relaboral[] = array(
                    'id_relaboral' => $v->id_relaboral,
    ...
    public function getintermediariospordepartamentoAction()
    {
        ...
        if ($palabraArea != '')
            ...
            $where = " relaborales.estado>=1 AND (cargos.cargo LIKE ''%" . $cargoSuperiorA . "%'' OR cargos.cargo LIKE ''" . $cargoSuperiorB . "%'' OR cargos.cargo LIKE ''%" . $cargoSuperiorC . "%'') AND cargos.cargo NOT LIKE ''JEF% DEPARTAMENTO%'' AND cargos.cargo NOT LIKE ''JEF% DPTO%'' ";
