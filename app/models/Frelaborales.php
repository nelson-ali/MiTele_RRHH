...
class Frelaborales extends \Phalcon\Mvc\Model
{
     ...
     * @return Resultset
     */
    public function getAllInGestion($gestion, $where = '', $group = '')
    {
        $sql = "SELECT * FROM f_relaborales_ultima_movilidad_por_gestion($gestion)";
        if ($where != '') $sql .= $where;
        if ($group != '') $sql .= $group;
        $this->_db = new Frelaborales();
        return new Resultset(null, $this->_db, $this->_db->getReadConnection()->query($sql));
     ...
     * @return Resultset
     */
    public function getPaged($opcion, $gestion, $where = '', $group = '', $offset, $limit)
    {
        if ($gestion >= 0) {
            $sql = "SELECT * FROM f_relaborales_ultima_movilidad_mas_nuevos_por_gestion($opcion,$gestion,'$where','$group',$offset,$limit)";
            /*echo "<p>----->".$sql;*/
            $this->_db = new Frelaborales();
            return new Resultset(null, $this->_db, $this->_db->getReadConnection()->query($sql));
        }
